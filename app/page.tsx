...
export default function Home(){
 ...
 useEffect(()=>{let selected:Language='vi';try{const saved=localStorage.getItem('truanayangi-language');selected=saved==='en'||saved==='vi'?saved:navigator.language.toLowerCase().startsWith('vi')?'vi':'en'}catch{selected=navigator.language.toLowerCase().startsWith('vi')?'vi':'en'}setLanguage(selected);document.documentElement.lang=selected;document.title=selected==='en'?'What should I eat for lunch?':'Trưa nay ăn gì?'},[]);
