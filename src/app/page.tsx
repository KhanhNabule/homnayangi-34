...
import {isProvince, matchesProvince, provinces, type Province} from '@/lib/provinces';
...
export default function Home(){
 ...
 const [budget,setBudget]=useState('50'),[custom,setCustom]=useState('50'),[province,setProvince]=useState<Province|'all'>('all'),[veg,setVeg]=useState(false),[sound,setSound]=useState(true),[spinning,setSpinning]=useState(false),[result,setResult]=useState<Food|null>(null),[revealed,setRevealed]=useState(false);
 ...
 useEffect(()=>{const saved=readCookie<Record<string,unknown>>('settings');if(saved&&typeof saved==='object'){if(typeof saved.budget==='string'&&['35','50','75','100','150','custom'].includes(saved.budget))setBudget(saved.budget);if(typeof saved.custom==='string'&&Number(saved.custom)>=30&&Number(saved.custom)<=180)setCustom(saved.custom);if(saved.province==='all'||isProvince(saved.province))setProvince(saved.province);if(typeof saved.veg==='boolean')setVeg(saved.veg);if(typeof saved.sound==='boolean')setSound(saved.sound)}setPreferencesReady(true)},[]);
 useEffect(()=>{if(preferencesReady){try{writeCookie('settings',{budget,custom,province,veg,sound});setCookieError('')}catch{setCookieError(language==='vi'?'Không thể lưu cookie. Lựa chọn chỉ giữ trong lần mở trang này.':'Cookies unavailable. Preferences last only for this visit.')}}},[preferencesReady,budget,custom,province,veg,sound,language]);
 ...
 const eligible=useMemo(()=>population.filter(f=>(!veg||f.veg)&&matchesProvince(f.province,province)),[population,veg,province]);
 ...
 {!eligible.length&&<p className="preferences-message">{language==='vi'?'Không có món phù hợp với tỉnh và bộ lọc hiện tại.':'No dishes match the selected province and filters.'}</p>}
