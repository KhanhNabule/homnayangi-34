...
- Store preferences in bounded, versioned, host-only cookies. Store user-selected dish images in local IndexedDB because they exceed cookie capacity. Validate imported/untrusted values and handle unavailable/full storage visibly.
...
- This app is for local use: bind dev/preview to loopback, use cookies for automatic preference persistence and IndexedDB only for user-selected dish images, and do not add login, account screens, backend endpoints, database clients or hosted-demo deployment. Only user-clicked external links may leave the local app; background asset loads stay local.
